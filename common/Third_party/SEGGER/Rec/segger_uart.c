...
#define GPIO_UART_TX_BITT    0UL                // USART2 TX: Pin PD8
#define GPIO_UART_RX_BITT    1UL                 // USART2 RX: Pin PD9
...
#define USART_ICR			*(volatile uint32_t*) (USART_BASE_ADDR + OFF_ICR)
#define USART_TDR            *(volatile uint32_t*)(USART_BASE_ADDR + OFF_TDR)
#define USART_RDR            *(volatile uint32_t*)(USART_BASE_ADDR + OFF_RDR)
#define USART_PRESC         *(volatile uint32_t*)(USART_BASE_ADDR + OFF_PRESC)
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  v  = GPIO_AFRH;
  v &=  ~((15UL << ((GPIO_UART_TX_BITT) << 2)) | (15UL << ((GPIO_UART_RX_BITT) << 2)));
  v |=   ((7UL << ((GPIO_UART_TX_BITT) << 2)) | (7UL << ((GPIO_UART_RX_BITT) << 2)));
  GPIO_AFRH = v;
  //
